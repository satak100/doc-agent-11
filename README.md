...
## Implemented prescription KB quick start

The costly OCR outputs and both built indexes are included under `artifacts/`.
To rebuild from the original images, point the two environment variables at the
image directory and final OCR JSONL, then run:

```bash
export PYTHONPATH=src
export DOC_AGENT_IMAGE_DIR=/path/to/images_310
export DOC_AGENT_OCR_JSONL=/path/to/final_output/results.jsonl
bash scripts/build_index.sh
python scripts/query_hybrid.py "Phoscon 210mg" -k 3
```

Open `notebooks/kb_demo.ipynb` for the OCR scores, index statistics, success
case, and failure case. See `configs/design_choices.md` for method selection and
limitations. Results are search aids only and require verification against the
