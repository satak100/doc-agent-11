...
def build_knowledge_base(cfg: dict) -> None:
    ...
    cfg["_page_id_to_doc"] = {page.id: page.doc_id for page in pages}
    ...
    text = hooks.run(hooks.AFTER_OCR, {"chunks": text})["chunks"]
    chunks = chunk.split(text, cfg)                 # Stage 4
    chunks = hooks.run(hooks.BEFORE_INDEX, {"chunks": chunks})["chunks"]
